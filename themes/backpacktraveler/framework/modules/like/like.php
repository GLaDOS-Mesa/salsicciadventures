...
class BackpackTravelerMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_backpacktraveler_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_backpacktraveler_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'backpacktraveler' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'backpacktraveler' );
		}
...
if ( ! function_exists( 'backpacktraveler_mikado_create_like' ) ) {
	function backpacktraveler_mikado_create_like() {
		BackpackTravelerMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'backpacktraveler_mikado_create_like' );
}