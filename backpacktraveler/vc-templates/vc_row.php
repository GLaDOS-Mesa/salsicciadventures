...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		$img_src = is_array($image_src) ? $image_src[0] : '';
		$mkdf_vc_row_style[] = 'background-image: url(' . esc_url( $img_src ) . ')';
	}
	...
	if ( ! empty( $parallax_background_image ) ) {
		...
		$wrapper_attributes[] = 'data-parallax-bg-image="' . esc_url( $img_src ) . '"';
	...
	if ( ! empty( $simple_background_image ) ) {
		$image_src              = wp_get_attachment_image_src( $simple_background_image, 'full' );
		$img_src = is_array($image_src) ? $image_src[0] : '';
		$mkdf_grid_row_style[] = 'background-image: url(' . esc_url( $img_src ) . ')';
	}
	...
	if ( ! empty( $parallax_background_image ) ) {
		...
		$grid_row_data[]  = 'data-parallax-bg-image=' . esc_url( $img_src );
...
if ( $row_content_width === 'grid' ) {
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . backpacktraveler_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
	$mkdf_row_wrapper_end   .= '</div></div>';
}

$mkdf_after_wrapper_open   = apply_filters('backpacktraveler_mikado_filter_vc_after_wrapper_open', '', $atts);
$mkdf_before_wrapper_close = apply_filters('backpacktraveler_mikado_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('backpacktraveler_mikado_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . backpacktraveler_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
...
print backpacktraveler_mikado_get_module_part($output);
