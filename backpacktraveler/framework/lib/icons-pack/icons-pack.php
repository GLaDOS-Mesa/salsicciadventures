...
foreach ( glob( MIKADO_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( backpacktraveler_mikado_is_customizer_item_enabled( $module_load, 'backpacktraveler_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'backpacktraveler_mikado_get_icon_pack_collections' ) ) {
	function backpacktraveler_mikado_get_icon_pack_collections() {
		$options = apply_filters( 'backpacktraveler_mikado_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'backpacktraveler_mikado_get_icon_pack_collections' );
}

if ( ! function_exists( 'backpacktraveler_mikado_get_icon_pack_options' ) ) {
	function backpacktraveler_mikado_get_icon_pack_options() {
		$options = apply_filters( 'backpacktraveler_mikado_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'backpacktraveler_mikado_get_icon_pack_options' );
}

if ( ! function_exists( 'backpacktraveler_mikado_get_icon_pack_social_options' ) ) {
	function backpacktraveler_mikado_get_icon_pack_social_options() {
		$options = apply_filters( 'backpacktraveler_mikado_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'backpacktraveler_mikado_get_icon_pack_social_options' );
}

/*
 * Class: BackpackTravelerMikadoClassIconCollections
 */
class BackpackTravelerMikadoClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = backpacktraveler_mikado_get_icon_pack_collections();
		$this->iconPackOptions       = backpacktraveler_mikado_get_icon_pack_options();
		$this->iconPackSocialOptions = backpacktraveler_mikado_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = backpacktraveler_mikado_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'backpacktraveler' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'backpacktraveler' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'backpacktraveler' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'backpacktraveler' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'backpacktraveler' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'backpacktraveler' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'backpacktraveler' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'backpacktraveler' )       => 'fa-lg',
			esc_html__( 'Small', 'backpacktraveler' )      => 'fa-2x',
			esc_html__( 'Medium', 'backpacktraveler' )     => 'fa-3x',
			esc_html__( 'Large', 'backpacktraveler' )      => 'fa-4x',
			esc_html__( 'Very Large', 'backpacktraveler' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iBackpackTravelerMikadoInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'backpacktraveler' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'backpacktraveler' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'backpacktraveler-mikado-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
    ...
    public function getElementorParamsArray($control_object, $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false){
        ...
        $control_object->add_control(
            'icon_pack', [
                'label' => esc_html__( 'Icon Pack', 'backpacktraveler' ),
            ...
            foreach ($this->iconCollections as $key => $collection) {
                $control_object->add_control(
                    $iconCollectionPrefix . $collection->param, [
                        'label' => esc_html__( 'Icon', 'backpacktraveler' ),
                        'type' => \Elementor\Controls_Manager::SELECT,
                        'options' => array_flip($collection->getIconsArray()),
                        'condition' => [
                            'icon_pack' => $key,
