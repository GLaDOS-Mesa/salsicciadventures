...
/*
   Class: BackpackTravelerMikadoClassDashboardForm
   A class that initializes Mikado Dashboard Container
*/

class BackpackTravelerMikadoClassDashboardForm implements iBackpackTravelerMikadoInterfaceLayoutNode, iBackpackTravelerMikadoInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render( $factory ) {
		...
		<form method="<?php echo esc_attr( $this->form_method ); ?>" id="<?php echo esc_attr( $this->form_id ); ?>" class="<?php echo esc_attr( $action_class ) ?>" data-action="<?php echo esc_attr( $this->form_action ); ?>">
			...
			if ( backpacktraveler_mikado_core_plugin_installed() ) {
				echo backpacktraveler_mikado_get_button_html( array(
					'html_type'    => 'button',
...
/*
   Class: BackpackTravelerMikadoClassDashboardGroup
   A class that initializes BackpackTravelerMikadoClass Group Field
*/
class BackpackTravelerMikadoClassDashboardGroup implements iBackpackTravelerMikadoInterfaceLayoutNode, iBackpackTravelerMikadoInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: BackpackTravelerMikadoClassDashboardTitle
   A class that initializes Dashboard Title
*/
class BackpackTravelerMikadoClassDashboardTitle implements iBackpackTravelerMikadoInterfaceRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: BackpackTravelerMikadoClassDashboardField
   A class that initializes BackpackTravelerMikadoClass Front Field
*/
class BackpackTravelerMikadoClassDashboardField implements iBackpackTravelerMikadoInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class BackpackTravelerMikadoClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class BackpackTravelerMikadoClassDashboardFieldText extends BackpackTravelerMikadoClassDashboardFieldType {
...
class BackpackTravelerMikadoClassDashboardFieldTextArea extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class BackpackTravelerMikadoClassDashboardFieldImage extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="mkdf-dashboard-gallery-holder">
				<ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						print backpacktraveler_mikado_get_module_part($value_html);
					} ?>
				</ul>
				<div class="mkdf-dashboard-gallery-uploader">
					<?php
					if ( backpacktraveler_membership_theme_installed() ) {
						echo backpacktraveler_mikado_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'backpacktraveler' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'backpacktraveler' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'backpacktraveler' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class BackpackTravelerMikadoClassDashboardFieldGallery extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class BackpackTravelerMikadoClassDashboardFieldSelect extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class BackpackTravelerMikadoClassDashboardFieldDate extends BackpackTravelerMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class BackpackTravelerMikadoClassDashboardFieldIcon extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = backpacktraveler_mikado_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = backpacktraveler_mikado_icon_collections()->getIconCollectionsKeys();
				...
				<div class="mkdf-dashboard-item">
					...
					<div class="mkdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = backpacktraveler_mikado_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
							...
							<div class="mkdf-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>">
									<?php
									$icons       = backpacktraveler_mikado_icon_collections()->getIconCollection( $icons_collection );
...
class BackpackTravelerMikadoClassDashboardFieldColor extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class BackpackTravelerMikadoClassDashboardFieldCheckBoxGroup extends BackpackTravelerMikadoClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class BackpackTravelerMikadoClassDashboardFieldAddress extends BackpackTravelerMikadoClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="mkdf-dashboard-field-row">
				<div class="mkdf-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
					<div class="mkdf-dashboard-input-holder">
						...
						<a class="mkdf-reset-marker mkdf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'backpacktraveler' ); ?></a>
...
class BackpackTravelerMikadoClassDashboardRepeater implements iBackpackTravelerMikadoInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'backpacktraveler' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-dashboard-repeater-table-heading">
					<div class="mkdf-dashboard-repeater-fields-holder">
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'backpacktraveler' ) ?></div>
						...
						<div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'backpacktraveler' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class BackpackTravelerMikadoClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new BackpackTravelerMikadoClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new BackpackTravelerMikadoClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new BackpackTravelerMikadoClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new BackpackTravelerMikadoClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new BackpackTravelerMikadoClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new BackpackTravelerMikadoClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new BackpackTravelerMikadoClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new BackpackTravelerMikadoClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new BackpackTravelerMikadoClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new BackpackTravelerMikadoClassDashboardFieldAddress();
